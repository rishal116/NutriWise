...
import {
  ...
  AddButton,
  PrimaryButton,
} from "@/components/auth/nutrtionistForm/Components";
  ...
  const validateField = (name: string, value: any) => {
    switch (name) {
      ...
      case "specializations":
        if (value.some((s: string) => !s.trim())) return "All specializations are required";
        break;
      case "experiences":
            ...
            {specializations.map((s, i) => (
              <DynamicInput
                ...
                onChange={(val) => {
                  ...
                  setErrors(prev => ({ ...prev, specializations: validateField("specializations", updated) }));
                }}
                onRemove={() => removeItem(setSpecializations, i)}
                canRemove={specializations.length > 1}
              />
            ))}
            <AddButton label="Add Another Specialization" onClick={() => addItem(setSpecializations)} />
          </Section>
