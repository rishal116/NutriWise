import { IChallengeMedia } from "@/types/challenge";
...
export function MediaItemCard({
  ...
  return (
    <div
      className={`rounded-2xl border transition-all duration-200 overflow-hidden ${hasErrors ? "border-red-200" : "border-slate-200"
        }`}
    >
      {/* ── Header ── */}
        ...
        <div className="p-4 space-y-4">
              ...
              {(Object.keys(MEDIA_TYPE_CONFIG) as MediaType[]).map((type) => {
                ...
                return (
                  <button
                    ...
                    type="button"
                    onClick={() => {
                      onChange({ ...item, type, url: "", file: undefined });
                      setPreviewUrl("");
                    }}
                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[11px] font-bold border transition-all duration-200 ${item.type === type
          ...
          <div>
            ...
            {previewUrl ? (
              ...
              <div
                onClick={() => fileRef.current?.click()}
                className={`flex flex-col items-center justify-center gap-2.5 py-8 border-2 border-dashed rounded-xl cursor-pointer transition-all duration-200 ${errs.url
            ...
            <div className="max-w-[180px]">
              ...
              <div className="relative">
                <input
                  type="number"
                  min={0}
                  className={`${inputCls} ${errs.duration ? inputErrCls : ""
                    } pr-8`}
                  placeholder="e.g. 120"
                  value={item.duration ?? ""}
                  onChange={(e) =>
