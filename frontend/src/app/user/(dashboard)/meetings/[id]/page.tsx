...
import type { UserMeetingDetailsResponseDTO } from "@/dtos/user/meeting/user-meeting-details-response.dto";

// Client-side join window — unlike the nutritionist (host) view, the client
// isn't the one starting the room, so we still gate how early they can walk in.
...
type JoinState =
  | { kind: "joinable" }
  | { kind: "too-early"; msUntilOpen: number }
  | { kind: "not-applicable" };

function getJoinState(
  meeting: UserMeetingDetailsResponseDTO,
  now: number,
): JoinState {
  if (
  ...
  // SCHEDULED — client can join up to EARLY_JOIN_MS before start
  const scheduledAtMs = new Date(meeting.scheduledAt).getTime();
  const windowOpensAt = scheduledAtMs - EARLY_JOIN_MS;

  if (now < windowOpensAt) {
    return { kind: "too-early", msUntilOpen: windowOpensAt - now };
  }
  return { kind: "joinable" };
}

export default function UserMeetingDetailsPage() {
  const router = useRouter();
  const params = useParams<{ id: string }>();
  const meetingId = params.id;

  const [meeting, setMeeting] = useState<UserMeetingDetailsResponseDTO | null>(
    null,
  );
  ...
  const loadMeeting = useCallback(async () => {
    try {
      setLoading(true);
      setError(false);
      const res = await userMeetingService.getMeetingDetails(meetingId);
      setMeeting(res.data);
    } catch {
      setError(true);
  ...
  // Tick every second while inside the last hour before the window opens,
  // otherwise every 30s — matches the listing page's countdown behavior.
  useEffect(() => {
    if (!meeting || meeting.status !== MeetingStatus.SCHEDULED) return;
    const scheduledAtMs = new Date(meeting.scheduledAt).getTime();
    const windowOpensAt = scheduledAtMs - EARLY_JOIN_MS;
    const fastTick = windowOpensAt - now < 3_600_000 && windowOpensAt > now;
  ...
  const joinState = getJoinState(meeting, now);
  const scheduledDate = new Date(meeting.scheduledAt);
  const showImage =
    meeting.nutritionist.profileImage && meeting.nutritionist.profileImage.length > 0;
...
function JoinPanel({
  ...
  if (joinState.kind === "too-early") {
    return (
      <div className="flex items-center justify-between gap-3 px-4 py-3.5 rounded-xl bg-sky-50 border border-sky-200 text-sm font-semibold text-sky-700">
        <span className="flex items-center gap-2.5">
          <Clock size={16} />
          Join opens in {formatCountdown(joinState.msUntilOpen)}
        </span>
      </div>
    );
  }
