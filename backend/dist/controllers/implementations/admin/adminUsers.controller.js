"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.AdminUsersController = void 0;
const asyncHandler_1 = require("../../../utils/asyncHandler");
const inversify_1 = require("inversify");
const types_1 = require("../../../types/types");
let AdminUsersController = class AdminUsersController {
    constructor(_adminUsersService) {
        this._adminUsersService = _adminUsersService;
        this.getAllClients = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
            const clients = await this._adminUsersService.getAllClients();
            res.status(200).json({ success: true, clients });
        });
        this.getAllNutritionist = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
            const nutritionists = await this._adminUsersService.getAllNutritionists();
            res.status(200).json({ success: true, nutritionists });
        });
        this.blockUser = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
            const { userId } = req.params;
            await this._adminUsersService.blockUser(userId);
            res.status(200).json({ success: true, message: "User blocked successfully" });
        });
        this.unblockUser = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
            const { userId } = req.params;
            await this._adminUsersService.unblockUser(userId);
            res.status(200).json({ success: true, message: "User unblocked successfully" });
        });
        this.getNutritionistDetails = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
            const { userId } = req.params;
            const nutritionist = await this._adminUsersService.getNutritionistById(userId);
            if (!nutritionist) {
                return res.status(404).json({ success: false, message: "Nutritionist not found" });
            }
            res.status(200).json({ success: true, nutritionist });
        });
    }
};
exports.AdminUsersController = AdminUsersController;
exports.AdminUsersController = AdminUsersController = __decorate([
    (0, inversify_1.injectable)(),
    __param(0, (0, inversify_1.inject)(types_1.TYPES.IAdminUsersService)),
    __metadata("design:paramtypes", [Object])
], AdminUsersController);
