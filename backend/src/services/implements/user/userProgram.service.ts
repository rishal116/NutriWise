...
export class UserProgramService implements IUserProgramService {
  ...
  async getPrograms(userId: string): Promise<ProgramResponseDTO[]> {
    logger.debug("Fetching programs for userId: %s", userId);

    const programs = await this._userProgramRepository.findByUser(
      new Types.ObjectId(userId)
    );

    logger.info("Found %d programs for userId: %s", programs.length, userId);
  ...
  async getProgramDetails(
    ...
    const program = await this._userProgramRepository.findByIdPopulated(programId);
  ...
  async getProgramDayDetails(
    ...
    if (!day) {
      logger.warn("Attempt to access non-existing program day: %s", dayId);
      throw new CustomError("Program day not found", StatusCode.NOT_FOUND);
    }

    await this.getProgramDetails(day.userProgramId.toString(), userId);
