...
import { INutritionistBrowsingService } from "../../../interfaces/user/discovery/INutriBrowsingService";
...
export class NutritionistBrowsingService implements INutritionistBrowsingService {
  ...
  private validateBrowseQuery(query: NutritionistListQueryDTO): void {
  ...
  async browseNutritionists(
    ...
    logger.info("Browsing nutritionists", { query });
    this.validateBrowseQuery(query);
    const result =
      await this._nutritionistBrowsingRepository.findNutritionists(query);
    logger.info("Nutritionists fetched successfully", {
      count: result.items.length,
    });
    return toNutritionistBrowseResponseDTO(result);
  }
