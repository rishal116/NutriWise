...
export const createSessionSchema = z
  ...
  .superRefine((data, ctx) => {
    if (data.type === "paid" && (!data.price || data.price < 50)) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["price"],
        message: "Paid sessions must have minimum price 50",
      });
    }

    if (data.type === "free" && data.price && data.price !== 0) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["price"],
...
export type CreateSessionSchemaDTO = z.infer<typeof createSessionSchema>;
