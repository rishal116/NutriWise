  ...
  implements IUserPlanRepository
{
  ...
  async findBySessionId(sessionId: string) {
    return this._model.findOne({ checkoutSessionId: sessionId }).exec();
  }

  async findActiveByUser(userId: string | Types.ObjectId) {
    return this._model
      .findOne({
        userId,
        status: "ACTIVE",
  ...
  async expireById(
    ...
    await this._model.findByIdAndUpdate(id, { status: "EXPIRED" }, { session });
  }
